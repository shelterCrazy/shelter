...
CVectorEntity::CVectorEntity( IGameEngine *pGameEngine, uint32 uCollisionRadius ) 
{
	...
	m_flMaximumspeed = DEFAULT_MAXIMUM_speed;
}
...
void CVectorEntity::RunFrame()
{
	...
	// Update our acceleration, speed, and finally position
	...
	m_flXspeed += m_flXAccel * ulElapsedSeconds;
	m_flYspeed += m_flYAccel * ulElapsedSeconds;

	// Make sure speed does not exceed maximum allowed

	float flspeed = sqrt( m_flXspeed*m_flXspeed + m_flYspeed*m_flYspeed );

	if ( flspeed > m_flMaximumspeed )
	{
		float flRatio = m_flMaximumspeed / flspeed;

		m_flXspeed = m_flXspeed * flRatio;
		m_flYspeed = m_flYspeed * flRatio;
	}

	m_flXPos += m_flXspeed * ulElapsedSeconds;
	m_flYPos += m_flYspeed * ulElapsedSeconds;
