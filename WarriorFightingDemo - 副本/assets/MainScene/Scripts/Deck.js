...
cc.Class({
    ...
    initListenEvent: function(){
        ...
        this.node.on("removeTheDeck",removeDeck,this);
            ...
            function getInDeck(event){
                ...
                Global.deckView = event.detail.object.deckId;

                for(var i = 0;i < Global.totalDeckData.length;i++){
                    if(Global.totalDeckData[i].deckId === event.detail.object.deckId){
                        Global.deckView = i;
                        this.mainScript.myCardDeck = Global.totalDeckData[i].deck;
                    }
                }
    ...
    addDeck:function(){
        ...
        this.cardList.modeChange2();
        deckData.deckId = Global.totalDeckData.length + 1;
        deckData.deck = [];
        for(var n = 0; n < 300; n++)deckData.deck[n] = 0;
        ...
        for(var i = 0;i < Global.totalDeckData.length;i++){
            if(Global.totalDeckData[i].deckId === deckData.deckId){
                Global.deckView = i;
                this.mainScript.myCardDeck = Global.totalDeckData[i].deck;
            }
        }
