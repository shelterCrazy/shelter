...
CShipDebris::CShipDebris( IGameEngine *pGameEngine, float xPos, float yPos, DWORD dwDebrisColor ) : CSpaceWarEntity( pGameEngine, 0, true )
{
	...
	float xspeed = GetXspeed() + ( sinvalue * 80 );
	float yspeed = GetYspeed() - ( cosvalue * 80 );
		...
		if ( pPhotonUpdate->GetActive() )
		{
			if ( !m_rgPhotonBeams[i] )
			{
				m_rgPhotonBeams[i] = new CPhotonBeam( m_pGameEngine, 
					...
					pPhotonUpdate->GetXspeed(), pPhotonUpdate->GetYspeed() );
			}
			else
			{
	...
	else if ( m_bIsServerInstance )
	{
		...
		if ( nNextAvailablePhotonBeamSlot != -1 && !m_bExploding && m_SpaceWarClientUpdateData.GetFirePressed() && ulCurrentTickCount - PHOTON_BEAM_FIRE_INTERVAL_TICKS > m_ulLastPhotonTickCount )
		{
			...
			if ( m_nShipWeapon == 1 ) // Item#101
			{
				...
				float xspeed = GetXspeed() + ( sinvalue1 * 275 );
				float yspeed = GetYspeed() - ( cosvalue1 * 275 );
				...
				m_rgPhotonBeams[nNextAvailablePhotonBeamSlot] = new CPhotonBeam( m_pGameEngine, xPos, yPos, m_dwShipColor, GetAccumulatedRotation(), xspeed, yspeed );
				...
				{
					xspeed = GetXspeed() + ( sinvalue2 * 275 );
					yspeed = GetYspeed() - ( cosvalue2 * 275 );
			...
			else
			{
				...
				float xspeed = GetXspeed() + ( sinvalue * speed );
				float yspeed = GetYspeed() - ( cosvalue * speed );
...
void CShip::BuildServerUpdate( ServerShipUpdateData_t *pUpdateData )
{
	...
	pUpdateData->SetXspeed( GetXspeed() );
	pUpdateData->SetYspeed( GetYspeed() );
...
void CShip::BuildServerPhotonBeamUpdate( ServerShipUpdateData_t *pUpdateData )
{
	for( int i = 0; i < MAX_PHOTON_BEAMS_PER_SHIP; ++i )
	{
		...
		if ( m_rgPhotonBeams[i] )
		{
			...
			pPhotonUpdate->SetXspeed( m_rgPhotonBeams[i]->GetXspeed() );
			pPhotonUpdate->SetYspeed( m_rgPhotonBeams[i]->GetYspeed() );
