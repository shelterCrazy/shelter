...
struct ServerPhotonBeamUpdateData_t
{
	...
	void SetXspeed( float flspeed ) { m_flXspeed = LittleFloat( flspeed ); }
	float GetXspeed() { return LittleFloat( m_flXspeed ); }

	void SetYspeed( float flspeed ) { m_flYspeed = LittleFloat( flspeed ); }
	float GetYspeed() { return LittleFloat( m_flYspeed ); }
...
private:
	...
	// The current speed for the ship
	float m_flXspeed;
	float m_flYspeed;
